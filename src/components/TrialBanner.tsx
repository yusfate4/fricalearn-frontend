  ...
  if (status.on_trial) {
    ...
    return (
      <div className={`rounded-2xl p-4 mb-6 flex flex-col sm:flex-row items-center justify-between gap-3 border-2 ${
        urgent ? "bg-orange-50 border-orange-200" : "bg-[#FFFF00]/10 border-[#FFFF00]/30"
      }`}>
        <div className="flex items-center gap-3">
          <Clock size={18} className={urgent ? "text-orange-500" : "text-[#FFFF00]"}/>
          <p className="text-xs font-black uppercase tracking-wide text-gray-700">
            Free trial: <span className={urgent ? "text-orange-600" : "text-[#3F2171]"}>{days} day{days !== 1 ? "s" : ""} left</span>
