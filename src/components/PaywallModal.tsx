      ...
      <div className="bg-white rounded-[2.5rem] max-w-lg w-full shadow-2xl relative animate-in zoom-in duration-300 my-8">
        ...
        {done ? (
          <div className="p-10 text-center">
            ...
            <h3 className="text-2xl font-black text-[#2A1650] uppercase italic tracking-tight mb-2">Premium Unlocked! 🎉</h3>
          ...
          <>
            ...
            <div className="bg-gradient-to-br from-[#2A1650] to-[#3F2171] p-8 rounded-t-[2.5rem] text-center">
              <p className="text-[#FFFF00] font-black text-[10px] uppercase tracking-widest mb-2">Free Trial Ended</p>
              ...
              <p className="text-4xl font-black text-[#FFFF00] italic mt-4">{price}<span className="text-white/40 text-sm font-medium"> / month</span></p>
            </div>
            ...
            <div className="p-8 space-y-5">
              ...
              <div className="flex bg-gray-100 p-1.5 rounded-2xl">
                <button onClick={() => setCurrency("NGN")}
                  className={`flex-1 py-3 rounded-xl font-black text-[10px] tracking-widest transition-all ${currency === "NGN" ? "bg-white text-[#3F2171] shadow" : "text-gray-400"}`}>
                  🇳🇬 PAY IN NAIRA
                </button>
                <button onClick={() => setCurrency("GBP")}
                  className={`flex-1 py-3 rounded-xl font-black text-[10px] tracking-widest transition-all ${currency === "GBP" ? "bg-white text-[#3F2171] shadow" : "text-gray-400"}`}>
                  🇬🇧 PAY IN POUNDS
                </button>
              </div>
              ...
              {bank && (
                <div className="bg-gray-50 rounded-2xl p-5 border-2 border-gray-100">
                  <div className="flex items-center gap-3 mb-3">
                    <Landmark size={18} className="text-[#3F2171]"/>
                    <p className="font-black text-gray-800 text-sm">{bank.flag} {bank.bank_name}</p>
                  </div>
                  <p className="text-2xl font-black text-[#3F2171] tracking-tight">{bank.account_number}</p>
              ...
              {/* Receipt upload */}
              <label className="block border-4 border-dashed border-gray-100 rounded-2xl p-6 text-center cursor-pointer hover:border-[#3F2171] transition-all">
                <input type="file" className="hidden" accept="image/*,application/pdf"
                  onChange={(e) => { setReceipt(e.target.files?.[0] || null); setError(null); }}/>
                <Upload size={24} className="text-[#3F2171] mx-auto mb-2"/>
