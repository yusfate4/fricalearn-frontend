...
import {
  ...
  GraduationCap, Video, BookOpen, Gift, Package,
} from "lucide-react";
...
export function AdminShell({ children, title }: { children: React.ReactNode; title: string }) {
  ...
  function NavItem({ item }: { item: { label: string; path: string; icon: any } }) {
        ...
        <div className="flex-1 overflow-y-auto">
          {GROUPS.map((group) => (
            <div key={group.label} className="mb-2">
              <p className={`px-2 pt-4 pb-1.5 text-[9px] font-black uppercase tracking-[0.2em] ${group.color}`}>
                {group.label}
              </p>
