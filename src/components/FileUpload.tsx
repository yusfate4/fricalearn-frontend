...
import { UploadCloud, FileText, Video, Image as ImageIcon, CheckCircle, AlertCircle, Loader2, X } from "lucide-react";
...
export default function FileUpload({ lessonId, onUploadSuccess }: FileUploadProps) {
  ...
  const handleUpload = async () => {
    ...
    try {
      await api.post(`/lessons/${lessonId}/content`, formData, {
        headers: {
      ...
      setMessage({ 
        type: "error", 
        text: error.response?.data?.message || "Failed to upload file. Please check the file size and type." 
      });
    } finally {
      setUploading(false);
    }
  ...
  return (
    <div className="bg-white p-8 rounded-[2rem] border-2 border-gray-50 shadow-sm">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h3 className="text-xl font-black text-gray-800">Upload Lesson Material</h3>
          <p className="text-gray-400 text-sm font-medium mt-1">Add videos, PDFs, or images to this lesson.</p>
        </div>
      </div>
      ...
      <div 
        className={`relative border-2 border-dashed rounded-[1.5rem] p-10 text-center transition-all ${
          file ? "border-frica-green bg-green-50/30" : "border-gray-200 hover:border-frica-green hover:bg-gray-50"
        }`}
      >
        <input
          type="file"
        ...
        {!file ? (
          <div className="pointer-events-none flex flex-col items-center">
            ...
            <p className="text-gray-600 font-bold">Click or drag file to this area to upload</p>
            <p className="text-gray-400 text-xs mt-2 uppercase tracking-widest font-bold">Max 100MB • MP4, PDF, PPT</p>
          </div>
        ) : (
          <div className="flex flex-col items-center">
            ...
            <p className="text-gray-400 text-sm font-bold mt-1">{(file.size / (1024 * 1024)).toFixed(2)} MB</p>
