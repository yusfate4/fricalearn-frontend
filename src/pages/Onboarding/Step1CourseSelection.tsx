  ...
  return (
    <Layout>
      <div className="max-w-7xl mx-auto px-6 py-10 md:px-12 md:py-16 animate-in fade-in duration-700">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-8 mb-8">
            <div>
              ...
              <p className="text-gray-500 font-bold text-sm max-w-2xl">
                Select subjects and <span className="text-[#2D5A27] font-black">one</span> free language course.
              </p>
            </div>
            ...
            <div className="flex flex-col items-end gap-2">
              <div className="flex bg-gray-100 p-1.5 rounded-[2.5rem] border-2 border-gray-200 shadow-inner">
                <button onClick={() => setCurrency("NGN")}
                  className={`px-8 py-4 rounded-[2rem] font-black text-[10px] tracking-widest transition-all ${currency === "NGN" ? "bg-white text-[#2D5A27] shadow-lg" : "text-gray-400"}`}>
                  🇳🇬 NAIRA (₦)
                </button>
                <button onClick={() => setCurrency("GBP")}
                  className={`px-8 py-4 rounded-[2rem] font-black text-[10px] tracking-widest transition-all ${currency === "GBP" ? "bg-white text-[#2D5A27] shadow-lg" : "text-gray-400"}`}>
                  🇬🇧 POUNDS (£)
                </button>
              </div>
        ...
        {/* Language notice */}
        {selectedLanguage && (
          <div className="mb-6 px-6 py-4 bg-[#F4B400]/10 border-2 border-[#F4B400]/30 rounded-2xl flex items-center gap-3">
            <Award size={18} className="text-[#F4B400]" />
            <p className="text-sm font-black text-gray-700 uppercase tracking-wide">
              1 free language selected: <span className="text-[#2D5A27]">{selectedLanguage.charAt(0).toUpperCase() + selectedLanguage.slice(1)}</span>
            ...
            return (
              <div key={course.id} onClick={() => toggleCourse(course.id)}
                className={`relative bg-white rounded-[2.5rem] md:rounded-[3rem] p-8 md:p-10 border-4 cursor-pointer transition-all duration-500 group ${
                  isSelected ? "border-[#2D5A27] shadow-2xl -translate-y-2" : "border-gray-100 hover:border-gray-200 shadow-sm"
                }`}>

                {/* Language — radio indicator */}
                {isLanguage && (
                  <div className="absolute top-6 right-6 bg-[#F4B400] text-white px-4 py-2 rounded-2xl flex items-center gap-2 shadow-lg">
                    <Award size={14}/>
                ...
                <div className="mb-6 flex justify-center">
                  <div className={`p-6 rounded-3xl ${isSelected ? "bg-[#2D5A27]/10" : "bg-gray-50"} transition-colors`}>
                    {course.id === "maths" ? <BookOpen size={40} className="text-blue-500"/>
                      ...
                      : <GraduationCap size={40} className="text-[#2D5A27]"/>}
                  </div>
                </div>
                ...
                <div className="text-center pt-6 border-t border-gray-100">
                  {course.scholarship ? (
                    <div>
                      ...
                      <p className="text-3xl font-black text-[#2D5A27] italic">FREE</p>
                      <p className="text-[8px] font-black uppercase tracking-widest text-[#F4B400] mt-2">Full Scholarship Applied</p>
                    </div>
                  ) : (
                    <div>
