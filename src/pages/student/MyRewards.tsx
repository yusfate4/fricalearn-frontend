...
export default function MyRewards() {
  ...
  // 🚀 Logic: Get the base URL for storage
  ...
  const fetchMyStuff = async () => {
    try {
      ...
      // res.data is expected to be an array of RewardRedemption objects
  ...
  const handleDownload = (filePath: string) => {
    ...
    // Ensure we don't double up on 'storage/' prefix
      ...
      <div className="max-w-6xl mx-auto p-6 md:p-12 animate-in fade-in duration-700">
        ...
        {loading ? (
          <div className="flex flex-col items-center justify-center py-32 opacity-20">
            ...
            <p className="font-black uppercase text-[10px] tracking-widest">Opening your chest...</p>
          </div>
        ) : (
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            {redemptions.map((item) => (
              ...
              >
                ...
                <div className="flex-1 text-center sm:text-left z-10">
                  ...
                  {/* 🚀 ACTION LOGIC: Only show download if FULFILLED */}
                  {item.status === "fulfilled" ? (
                    item.reward?.type === "digital_asset" ? (
                      <button
            ...
            {redemptions.length === 0 && (
              <div className="col-span-full py-32 bg-gray-50 rounded-[4rem] border-4 border-dashed border-gray-100 text-center flex flex-col items-center">
                ...
                <h3 className="text-2xl font-black text-gray-800 uppercase italic tracking-tighter mb-2">Chest is Empty</h3>
                <p className="text-gray-400 font-bold uppercase text-[10px] tracking-[0.2em] mb-8">Go earn some XP in your lessons!</p>
                <Link to="/courses" className="bg-[#2D5A27] text-white px-10 py-4 rounded-2xl font-black uppercase text-[10px] tracking-widest hover:scale-105 transition-all">
                    Start Learning
                </Link>
              </div>
            )}
...
function Trophy({ size, className }: any) {
    return (
      <svg width={size} height={size} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" className={className}>
        <path d="M6 9H4.5a2.5 2.5 0 0 1 0-5H6" /><path d="M18 9h1.5a2.5 2.5 0 0 0 0-5H18" /><path d="M4 22h16" /><path d="M10 22V18" /><path d="M14 22V18" /><path d="M12 18a8 8 0 0 1-8-8V4h16v6a8 8 0 0 1-8 8Z" />
      </svg>
    );
}